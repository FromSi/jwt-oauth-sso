...
func Test_NewSuccessRefreshResponse(t *testing.T) {
	config := configs.NewBaseConfig()
	device := repositories.NewGormDevice()
	response, err := NewSuccessRefreshResponse(config, device)
	...
	response, err = NewSuccessRefreshResponse(config, deviceMock)
